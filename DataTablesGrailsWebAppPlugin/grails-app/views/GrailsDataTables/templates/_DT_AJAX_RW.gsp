%{--
    Implements the rendering of datatable with editor, given data from taglib
--}%
...
<div data-webui="ajax-table-${uuid}" style="display: none">
    <table id="webui-ajax-datatable-${uuid}-${id}" class="display" style="width: 100%">
        <thead>
        <tr>
        ...
        for (let i = 0; i < x.length; i++) {
            ...
            if (table != null) {
                let editor = new DataTable.Editor( {
                    ...
                    table: "#webui-ajax-datatable-${uuid}-${id}",
                    actionName: 'webui_editor_action',
                    fields: [<g:each in="${ajaxColumnNames}" var="ajaxColumnName" status="i">
                        <g:if test="${editableAjaxColumnNames.contains(ajaxColumnName)}">
                            {
                                label: "${columns.get(i)}",
                                name: "${grailsDataTablesService.gspBase64(ajaxColumnNames.get(i))}",
                                type: "${editableAjaxColumnFieldTypes.get(ajaxColumnNames.get(i))}"
                            },
                        </g:if>
                        </g:each>
                    ],
                ...
                new DataTable(table, {
                    ...
                    layout: {
                        topStart: {
                            ...
                            buttons: [
                                <g:if test="${wantedCred?.contains("new")}">
                                {extend: 'create', editor: editor},
                                </g:if>
                                <g:if test="${wantedCred?.contains("edit")}">
                                {extend: 'edit', editor: editor},
                                </g:if>
                                <g:if test="${wantedCred?.contains("delete")}">
